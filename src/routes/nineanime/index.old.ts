...
nineanimeRouter.get("/episodes/:id", async (c) => {
  try {
    const id = c.req.param("id");

    // Fetch watch page to get internal ID
    const watchUrl = `https://nineanime.com/watch/${id}`;
    ...
    const ajaxUrl = `https://9animetv.to/ajax/episode/list/${internalId}`;
...
// ============================================
// Single Episode Link Endpoint
// ============================================

nineanimeRouter.get("/episode/:id/:ep", async (c) => {
  try {
    ...
    // Fetch episode list
    const ajaxListUrl = `https://9animetv.to/ajax/episode/list/${internalId}`;
    const listResponse = await fetch(ajaxListUrl, {
      headers: {
        "X-Requested-With": "XMLHttpRequest",
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
    ...
    const episodeListHtml = await listResponse.text();
    ...
    const serversUrl = `https://9animetv.to/ajax/episode/servers?episodeId=${episodeId}`;
    ...
    const sourcesUrl = `https://9animetv.to/ajax/episode/sources?id=${serverId}`;
